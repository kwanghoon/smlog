...
datatype D = C of unit

datatype OD = datatype D

datatype LD = LD of (Ostring * LD Option.option ref) | LC of Lunit

fun EMBEDLD (C ARGt)  = (LC (EMBEDLunit ARGt)
)

and PROJLD (LD (_, (ref Option.NONE)))  = raise SMLOG.ProjectionError
| PROJLD (LD (_, (ref (Option.SOME ARGt))))  = (PROJLD ARGt)

| PROJLD (LC ARGt)  = (C (PROJLunit ARGt)
)
...
| OCCURLD ARG0 (LC ARG1)  = (OCCURLunit ARG0 ARG1)
...
| UNIFYLD ((LC ARGt1), (LC ARGt2)) ARGtc  = (UNIFYLunit (ARGt1, ARGt2) ARGtc)


fun PRINTLD (LD (ARGs, (ref Option.NONE)))  = ARGs
| PRINTLD (LD (_, (ref (Option.SOME ARGt))))  = (PRINTLD ARGt)

| PRINTLD (LC ARGt)  = (String.concat ["C", " ", (PRINTLunit ARGt)
])
